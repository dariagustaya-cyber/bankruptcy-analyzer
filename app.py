...
def bo_get_financials(org_id, detail_id):
    ...
    try:
        lines = _parse_lines(safe_get(f"{base}/nbo/organizations/{org_id}/bfo/{detail_id}/balance").json())
    ...
    try:
        lines = _parse_lines(safe_get(f"{base}/nbo/organizations/{org_id}/bfo/{detail_id}/financialResult").json())
...
def main():
    ...
    if "ИНН" in mode:
        ...
        if run and inn and len(inn) >= 10:
            ...
            # ── Компания ──
            with st.spinner("🔍 Ищу компанию..."):
                org = bo_search(inn)
            if not org:
                st.error("❌ Компания не найдена"); return

            age = age_over if age_over > 0 else calc_age(org.get("regDate"))

            st.markdown(f"### 🏢 {org['name']}")
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("ИНН", org["inn"]); m2.metric("ОГРН", org.get("ogrn", "—"))
            m3.metric("Статус", org.get("status", "—"))
            ...
            if not periods:
                st.warning("Периоды не найдены"); return
            ...
            if not fns:
                st.warning("Финансовые данные не найдены"); return

            st.success(f"✅ Загружено {len(fns)} показателей за {sel} г.")

            with st.expander("📋 Показатели (тыс. руб.)"):
                dd = {k: f"{v:,.0f}" for k, v in fns.items() if isinstance(v, (int, float))}
                st.dataframe(pd.DataFrame(dd.items(), columns=["Показатель", "Значение"]),
            ...
            with t_arb:
                with st.spinner("⚖️ Загружаю дела..."):
                    arb = kad_search(inn)
                if arb.get("error"):
                    st.warning(f"Ошибка: {arb['error']}")
                    st.info(f"[Проверьте вручную →](https://kad.arbitr.ru/)")
                else:
